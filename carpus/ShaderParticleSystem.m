...
@implementation ShaderParticleSystem  {
    // Instance variables
    GLKVector2  _pos;
    GLKVector2  _gravity;
    float       _life;
...
- (void) draw:(Camera*)camera {
    // Uniforms
    float intensity = 2 * M_PI;
    GLKMatrix4 modelViewProjectionMatrix = GLKMatrix4Multiply(camera.projectionMatrix, camera.baseModelViewMatrix);
    glUniform1f(camera.particleShader.u_Intensity, intensity);
    glUniformMatrix4fv(camera.particleShader.u_ProjectionMatrix, 1, 0, modelViewProjectionMatrix.m);
    glUniform2f(camera.particleShader.u_Gravity, _gravity.x, _gravity.y);
    glUniform1f(camera.particleShader.u_Time, _time);
    glUniform1f(camera.particleShader.u_eRadius, radius);
    glUniform1f(camera.particleShader.u_eVelocity, velocity);
    glUniform1f(camera.particleShader.u_eDecay, decay);
    glUniform1f(camera.particleShader.u_eSize, size);
    glUniform1f(camera.particleShader.u_eLife, _life);
    glUniform3f(camera.particleShader.u_eColor, particleColor.r, particleColor.g, particleColor.b);
    glUniform2f(camera.particleShader.u_ePosition, _pos.x, _pos.y);
